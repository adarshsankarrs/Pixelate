...
def welcome():
    ...
    + 'filters to achieve different styles of pixelation effects. '
...
def upload():
    ...
    if uploaded_file is not None:
        if uploaded_file.name[-3:] == "mp4" or uploaded_file.name[-3:] == "mkv":
            ...
            # Calling the Pixelate_video function
            Pixelate_video(vf)
...
def webcam():
    ...
    # Sends captured image to Pixelate
    webcam_cartoon = Pixelate_withoptions(image)
...
def Pixelate_video(vf):
    """"
    Function to split videos frame by frame, apply desired pixelation
    ...
    quant = st.selectbox(
        'Specify whether color quantisation is to be used for pixelation',
        ('No', 'Yes')
    )
    ...
    while(True):
        ...
        if success:
            frame_cartoon = Pixelate(frame, blur, edge, erosion, quant, k, filter)
...
def Pixelate_withoptions(image):
    """
    Function to edit options of pixelation effect in real time
    """
    st.header("pixelation")
...
def Pixelate(image, blur, edge, erosion, quant, k, filter):
    """
    Funcation to Pixelate with preset options
    """
    #print(image.shape)
    image_gray=cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
